...
class frmRotar(DialogType, DialogBase):
    ...
    def __init__(self, mapCanvas, conn, ftrs_nodos, ftrs_lineas, ftrs_postes, ftrs_areas, ftrs_parcelas, ftrs_ejes, ftrs_cotas, ftrs_anotaciones, pc):
        super().__init__()
        self.setupUi(self)
                    ...
                    for ftr in ftrs:
                        ...
                        if mx!=0 or my!=0:
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Nodos')
                            lyr.triggerRepaint()
                    ...
                    for ftr in ftrs:
                        ...
                        for i in range(m):
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Lineas')
                            lyr.triggerRepaint()
                    ...
                    for ftr in ftrs:
                        ...
                        for i in range(m):
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Ejes')
                            lyr.triggerRepaint()
                    ...
                    for ftr in ftrs:
                        ...
                        if mx!=0 or my!=0:
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Postes')
                            lyr.triggerRepaint()

                if lyr.name() == 'Anotaciones':
                    ftrs = lyr.getFeatures(self.ftrs_anotaciones)
                    for ftr in ftrs:
                        mx, my = self.calcular_giro(ftr.geometry().asPoint().x(), ftr.geometry().asPoint().y(), pc.x(), pc.y(), angulo, sentido)
                        if mx!=0 or my!=0:
                            cnn = self.conn
                            cursor = cnn.cursor()
                            try:
                                cursor.execute("mover_anotacion " + str(ftr.id()) + ', ' + str(mx) + ', ' + str(my))
                                cnn.commit()
                            except:
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Anotaciones')
                            lyr.triggerRepaint()
                    ...
                    for ftr in ftrs:
                        ...
                        for i in range(n):
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Areas')
                            lyr.triggerRepaint()
                    ...
                    for ftr in ftrs:
                        ...
                        for i in range(n):
                            ...
                            try:
                                ...
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Parcelas')
                            lyr.triggerRepaint()

                if lyr.name() == 'Cotas':
                    ftrs = lyr.getFeatures(self.ftrs_cotas)
                    for ftr in ftrs:
                        vertices  = ftr.geometry().asPolyline()
                        m = len(vertices)
                        #QMessageBox.information(None, 'vertices', str(m))
                        for i in range(m):
                            ...
                            cursor = cnn.cursor()
                            try:
                                cursor.execute("mover_cota " + str(ftr.id()) + ', ' + str(i) + ', ' + str(mx) + ', ' + str(my))
                                cnn.commit()
                            except:
                                cnn.rollback()
                                QMessageBox.critical(None, 'EnerGis 5', 'No se pudieron rotar Cotas')
                            lyr.triggerRepaint()
        n = self.mapCanvas.layerCount()
        layers = [self.mapCanvas.layer(i) for i in range(n)]
        for lyr in layers:
