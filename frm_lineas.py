...
class frmLineas(DialogType, DialogBase):
    ...
    def __init__(self, tipo_usuario, mapCanvas, conn, tension, nodo_desde, nodo_hasta, longitud, obj, geoname):
        ...
        if self.geoname != 0:
            ...
            cursor = cnn.cursor()
            cursor.execute("SELECT Descripcion, Val1 AS I, Val2 AS Seccion, Val3 AS R1, Val4 AS X1, Val5 AS R0, Val6 AS X0, Val7 AS Xc, Fase, Elmt, Desde, Hasta, Longitud, Tension, Zona, Alimentador, Modificacion, Exp, Disposicion, Conservacion, Ternas, UUCC, Acometida, Lineas.Estilo, Elementos_Lineas.Estilo FROM Lineas LEFT JOIN Elementos_Lineas ON Lineas.Elmt=Elementos_Lineas.id WHERE Geoname=" + str(self.geoname))
            #convierto el cursor en array
            datos_linea = tuple(cursor)
            cursor.close()
            if datos_linea[0][0]==-1: #Es virtual
                self.rbtVirtual.setChecked(True)
                self.liwElementos.setEnabled(False)
                self.txtLongitud.setText('0')
            else:
                ...
                #Seteo Capa -> Cargo Lista -> Marco Foco
        ...
        self.rbtVirtual.toggled.connect(self.linea_virtual)
        self.rbtReal.toggled.connect(self.linea_real)
        ...
        self.chkAcometida.stateChanged.connect(self.lleno_lista)
    ...
    def lleno_lista(self):
        ...
        cnn = self.conn
        cursor = cnn.cursor()
        if int(self.cmbCapa.currentText()) > 50000:
            cursor.execute("SELECT id, Descripcion, Estilo FROM Elementos_Lineas WHERE uso=1 OR uso=2")
        elif int(self.cmbCapa.currentText()) > 1000 and int(self.cmbCapa.currentText()) <= 50000:
            cursor.execute("SELECT id, Descripcion, Estilo FROM Elementos_Lineas WHERE uso=1 OR uso=3")
        elif int(self.cmbCapa.currentText()) <= 1000:
            if self.chkAcometida.isChecked():
                cursor.execute("SELECT id, Descripcion, Estilo FROM Elementos_Lineas WHERE uso=1 OR uso=5")
            else:
                cursor.execute("SELECT id, Descripcion, Estilo FROM Elementos_Lineas WHERE uso=1 OR uso=4")
        #convierto el cursor en array
    ...
    def linea_virtual(self):
        self.liwElementos.setEnabled(False)
        self.txtLongitud.setText('0')

    def linea_real(self):
        self.liwElementos.setEnabled(True)
        self.txtLongitud.setText(str(format(self.longitud, ",.2f")).replace(',',''))
    ...
    def aceptar(self):
        ...
        unidad = str_matriz[4]
        if self.estilo_catalogo!=None:
            str_matriz = self.estilo_catalogo.split("-")
            estilo = str_matriz[2]
        if self.rbtVirtual.isChecked()==True:
            self.elmt=-1
