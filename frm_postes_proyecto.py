...
DialogBase, DialogType = uic.loadUiType(os.path.join(os.path.dirname(__file__),'frm_postes_proyecto.ui'))

class frmPostesProyecto(DialogType, DialogBase):
        
    def __init__(self, proyecto, tipo_usuario, mapCanvas, conn, tension, geoname, elemento_asociado, geoname_asociado, zona, punto):
        super().__init__()
        self.setupUi(self)
        ...
        for row in rows:
            ...
            self.cmbRienda.addItem(str(row[1]))
            if str(row[1])=="Sin Rienda":
                b_existe=True
        if b_existe==False:
        ...
        if self.geoname != 0:
            cursor = cnn.cursor()
            cursor.execute("SELECT profundidad AS Tension FROM Postes WHERE geoname=" + str(self.geoname))
            tensiones = tuple(cursor)
            cursor.close()
            self.tension = tensiones[0][0]

        #QMessageBox.information(None, 'EnerGis 5', self.tension)
        ...
        for lyr in layers:
            if lyr.name()[:5] == 'Nodos': #A propósito uso nodos, los postes suelen estar apagados
        ...
        if self.geoname != 0:
            ...
            cursor = cnn.cursor()
            cursor.execute("SELECT Estructura,CASE WHEN Riendas.id IS NULL THEN 'Sin Rienda' ELSE Riendas.descripcion END AS Rienda,Altura,Nivel,Tension,Zona,Modificacion,tipo,Aislacion,Fundacion,Comparte,Ternas,PAT,Descargadores,elmt,profundidad FROM Postes LEFT JOIN Elementos_Postes ON Postes.Elmt=Elementos_Postes.id LEFT JOIN Riendas ON Postes.rienda=Riendas.id WHERE Geoname=" + str(self.geoname))
            #convierto el cursor en array
            datos_poste = tuple(cursor)
            cursor.close()
            for i in range (0, self.cmbCapa.count()):
                if self.cmbCapa.itemText(i) == str(datos_poste[0][15]):
            ...
            for i in range (0, self.cmbRienda.count()):
                if self.cmbRienda.itemText(i) == str(datos_poste[0][1]):
                    self.cmbRienda.setCurrentIndex(i)
            self.txtAltura.setText(str(datos_poste[0][2]))
            self.txtCota.setText(str(datos_poste[0][3]))
            self.str_zona=str(datos_poste[0][5])
    ...
    def agregar(self):
        ...
        dialogo = frmAbmPostes(self.conn, self.tipo_usuario, apoyo, estructura, rienda)
        dialogo.exec()
    ...
    def aceptar(self):
        ...
        if self.geoname == 0: #Si es nuevo -> INSERT
            ...
            str_valores = str_valores + "'" + self.cmbCapa.currentText() + "', " #profundidad (tension)
            ...
            str_valores = str_valores + ", 0"
            try:
                cursor.execute("INSERT INTO Postes (Geoname,id_nodo,XCoord,YCoord,elmt,estilo,estructura,rienda,altura,nivel,zona,tension,tipo,aislacion,fundacion,comparte,ternas,modificacion,descripcion,profundidad,pat,descargadores,obj,giro) VALUES (" + str_valores + ")")
                cnn.commit()
                ...
                cnn.rollback()
                QMessageBox.critical(None, 'EnerGis 5', 'No se pudo insertar en la Base de Datos - INSERT INTO Postes (Geoname,id_nodo,XCoord,YCoord,elmt,estilo,estructura,rienda,altura,nivel,zona,tension,tipo,aislacion,fundacion,comparte,ternas,modificacion,descripcion,profundidad,pat,descargadores,obj,giro) VALUES (' + str_valores + ')')
            ...
            str_set = str_set + "zona='" + self.str_zona + "', "
            ...
            str_set = str_set + "modificacion='', " #fecha

            str_set = str_set + "descripcion='" + self.proyecto + "', "
            str_set = str_set + "profundidad='" + self.cmbCapa.currentText() + "', "
