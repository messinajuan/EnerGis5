...
class frmPostesProyecto(DialogType, DialogBase):
    ...
    def __init__(self, proyecto, tipo_usuario, mapCanvas, conn, tension, geoname, elemento_asociado, geoname_asociado, zona):
        super().__init__()
        self.setupUi(self)
        ...
        if self.geoname != 0:
            ...
            cursor = cnn.cursor()
            cursor.execute("SELECT Estructura,Rienda,Altura,Nivel,Tension,Zona,Modificacion,tipo,Aislacion,Fundacion,Comparte,Ternas,PAT,Descargadores,elmt,profundidad FROM Postes LEFT JOIN Elementos_Postes ON Postes.Elmt=Elementos_Postes.id WHERE Geoname=" + str(self.geoname))
            #convierto el cursor en array
    ...
    def aceptar(self):
        ...
        if self.geoname == 0: #Si es nuevo -> INSERT
            ...
            try:
                cursor.execute("INSERT INTO Postes (Geoname,id_nodo,XCoord,YCoord,elmt,estilo,estructura,rienda,altura,nivel,zona,tension,tipo,aislacion,fundacion,comparte,ternas,modificacion,descripcion,profundidad,pat,descargadores,obj) VALUES (" + str_valores + ")")
                cnn.commit()
